...
#[test]
fn simulates_iww() {
    simulate_path("../../test/i2model015-simplifiedmay24-user1.stmx");
}
...
fn simulate_path(xmile_path: &str) {
    ...
    let (datamodel_project, sim, results1) = {
        ...
        println!("{:?}", datamodel_project.models);
        match datamodel_project.get_model_mut("main") {
            Some(model) => {
                println!("{:?}", model);
                if let Some(q051) = model.get_variable_mut("q051") {
                    println!("{:?}", q051);
                    println!("{:?}", q051.get_equation());
                    q051.set_scalar_equation("0.1");
                    println!("{:?}", q051);
                }
            }
            None => todo!(),
